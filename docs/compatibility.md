...
## At a glance
    ...
    architecture       x86-64                                            declared, never observed
...
**Observed: arm64 only.** Every recorded load and attach of the programs was on arm64. The x86-64
programs are compiled and committed, and pass the build-time checks, but no recorded run has loaded
them on an x86-64 host.
